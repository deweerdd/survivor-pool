...
- [x] **CSRF protection on server actions** (2026-04-13) — Verified Next.js 16's built-in server-action `Origin`-header check is active. `next.config.ts` sets no `experimental.serverActions.allowedOrigins` override, so defaults apply: cross-origin server-action POSTs are rejected. Documented in `decisions.md`.
